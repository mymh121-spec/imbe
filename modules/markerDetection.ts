...
import { CameraBatonRenderer, type CameraBatonOverlay } from './cameraBatonOverlay';
...
export function computeCameraBatonOverlay(markers: DetectedMarker[]): CameraBatonOverlay | null {
  ...
  return {
    anchor,
    direction,
    gripScale: measurements.reduce((sum, measurement) => sum + measurement.size, 0) / measurements.length,
  };
}
...
export class MarkerDetection {
  ...
  private drawPreview(canvas: HTMLCanvasElement, image: ImageData, markers: DetectedMarker[]) {
    ...
    this.batonRenderer.draw(context, canvas, computeCameraBatonOverlay(markers));
  }
}
